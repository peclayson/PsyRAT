classdef TestSubjDiffDynrelTrtRescorNativeHmcRecovery < PsyRATTestBase
    ...
    % 20; dynrel=2 + diffest=2 + sserrvar=2 + diffwpcov=2 + a time/occasion facet).
    % This is the concurrent one-facet subject-level model (case 19) with the
    % single trial 2-D block generalized to the five crossed cross-condition 2-D
    % factors (trl/occ/tid/oid/to, as case 16); the person (id) factor stays the
    % 4-D joint location+scale block. Matches
    % psyrat_build_stan_diffdynrel_sserr_trt_rescor.
    %
    % Generator uses INDEPENDENT person/facet event LOCATION effects (so the
    % difference universe variance recovers at modest N) plus the per-person
    % residual-scale random effect (the defining subject-level quantity) and the
    % bivariate residual correlation. It is the MOST parameter-rich concurrent
    % native model (4-D person block + five crossed factors + bivariate residual),
    % so it is slow.
    %
    ...
    methods (Test)
        function testRecoversTruth(testCase)
            ...
            % ---- recovery (generous bands): universe variance + rescor + the
            % per-subject residual-scale SDs (sd_id columns 3-4) ----
            uniHat = mean(idvc(:,1,1) + idvc(:,2,2) - 2*idvc(:,2,1));
            testCase.verifyEqual(uniHat, uniTrue, 'AbsTol', max(8.0, 0.4*uniTrue));
            testCase.verifyEqual(mean(rc), rho, 'AbsTol', 0.22);
            testCase.verifyEqual(mean(bsg(:,1)), a(1), 'AbsTol', 0.30);
            testCase.verifyEqual(mean(bsg(:,2)), a(2), 'AbsTol', 0.30);
            testCase.verifyEqual(mean(bsd(:,1)), bsig(1), 'AbsTol', 0.24);
            testCase.verifyEqual(mean(bsd(:,2)), bsig(2), 'AbsTol', 0.24);
