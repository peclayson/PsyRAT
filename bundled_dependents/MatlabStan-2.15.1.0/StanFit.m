...
classdef StanFit < handle
   ...
   methods
      ...
      function process_exit_success(self,src)
         ...
         % PsyRAT local patch (base-MATLAB compatibility, B39): nansum is a
         % Statistics and Machine Learning Toolbox function, so these two lines
         % made the DEFAULT CmdStan path require that toolbox for EVERY design,
         % not for the three the support matrix documents by name. self.loaded is
         % nan(size(self.output_file)) (see the constructor) with entries set to
         % true as each chain loads, and nansum(x) and sum(x,'omitnan') both treat
         % NaN as zero, so the replacement is behavior-preserving on every path.
         % This site runs inside a timer callback, where MATLAB downgrades an
         % undefined-function error to a warning, so on base MATLAB it degraded
         % silently; the companion site in block() is what actually aborted the
         % run, after sampling had already been paid for. Regression pin:
         % tests/TestBaseMatlabOnly.m, which now scans bundled_dependents/ (B40).
         if sum(self.loaded,'omitnan') == numel(self.loaded)
      ...
      function block(self)
         ...
         if ~isempty(self.processes)%is_running(self) % stan called
            ...
            % PsyRAT local patch (base-MATLAB compatibility, B39) - see the
            % full note at the sibling site in the chain-load callback above.
            % This is the site on the main execution stack, and therefore the
            % one that actually killed a base-MATLAB run.
            while sum(self.loaded,'omitnan') ~= numel(self.loaded)
