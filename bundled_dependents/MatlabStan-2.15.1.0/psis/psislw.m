...
for i1=1:size(lw,2)
    ...
    n=numel(x);
    % PsyRAT local patch (base-MATLAB compatibility, B40): prctile is a
    % Statistics and Machine Learning Toolbox function. MATLAB documents
    % prctile(x,p) as quantile(x,p/100). Checked rather than taken on the
    % documentation's word: bit-identical over 200 random vectors (n 3-500,
    % random scale, random p) and at p = 0/25/50/75/100. Nothing in PsyRAT
    % calls psislw - the file is unreachable vendored code - but the
    % base-MATLAB scan in tests/TestBaseMatlabOnly.m now covers
    % bundled_dependents/, and patching the one site is preferable to adding
    % a scan exemption, which would reintroduce the blind spot B40 exists to
    % close.
    xcutoff=quantile(x,(100-wcpp)/100);
