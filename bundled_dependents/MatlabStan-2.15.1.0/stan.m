...
%              make build STAN_THREADS=true before recompiling the model.
%              Default = false.
...
p.addParamValue('stan_home','',@(x) ischar(x) || isstring(x));
p.parse(varargin{:});

if isempty(p.Results.fit)
   if isempty(p.Results.stan_home)
      model = StanModel();
   else
      model = StanModel('stan_home',char(p.Results.stan_home));
   end
...
if ~isempty(p.Results.stan_home)
   model.stan_home = char(p.Results.stan_home);
end
