...
classdef TestStanModel < TestCase
   ...
   methods
      ...
      function testThreadsPerChainManualAndOff(self)
         s = StanModel('model_code',self.bernoulli_code,'file_overwrite',true);
         s.set('method','sample','chains',2,'threads_per_chain',2);
         cmd = sprintf('%s',s.command{:});
         
         if self.version_at_least(s.stan_version,[2 28 0])
            assertTrue(isfield(s.params,'num_threads'));
            assertEqual(s.params.num_threads,2);
            assertTrue(~isempty(strfind(cmd,'num_threads=2')));
         else
            assertFalse(isfield(s.params,'num_threads'));
            assertTrue(isempty(strfind(cmd,'num_threads=')));
         end
         
         s.set('threads_per_chain',1,'rebuild_for_threads',true);
         cmd = sprintf('%s',s.command{:});
         assertTrue(s.rebuild_for_threads);
         assertFalse(isfield(s.params,'num_threads'));
         assertTrue(isempty(strfind(cmd,'num_threads=')));
      end
      
      function testThreadsPerChainAutoResolution(self)
         s = StanModel('model_code',self.bernoulli_code,'file_overwrite',true);
         n_processors = java.lang.Runtime.getRuntime.availableProcessors;
         
         s.set('method','sample','chains',3,'threads_per_chain','auto');
         expected = max(1,floor(n_processors/3));
         
         if self.version_at_least(s.stan_version,[2 28 0]) && (expected > 1)
            assertTrue(isfield(s.params,'num_threads'));
            assertEqual(s.params.num_threads,expected);
         else
            assertFalse(isfield(s.params,'num_threads'));
         end
      end
   ...
   methods(Static)
      ...
      function bool = version_at_least(ver,req)
         bool = false;
         for i = 1:3
            if ver(i) > req(i)
               bool = true;
