      ...
      function out = extract(self,varargin)
         ...
         if p.Results.permuted
            ...
            for i = 1:numel(names)
               ...
               sz = size(temp);
               % PsyRAT local patch (upstream indexing bug): permute the DRAW
               % dimension, sz(1), not max(sz). permute_index has length equal
               % to the total post-warmup draws, so for any parameter with a
               % single dimension larger than the draw count (e.g. matrix[N,2]
               % with N > draws) the original 1:max(sz) overran it and extract
               % errored after a completed fit. Whenever the original
               % expression succeeded, max(sz) == sz(1) (a permutation of
               % 1..L can only index rows 1..sz(1) if max(sz) <= sz(1), and
               % max(sz) >= sz(1) always), so this line is bit-identical on
               % every previously working extraction. Regression test:
               % tests/TestMatlabStanExtractBound.m.
