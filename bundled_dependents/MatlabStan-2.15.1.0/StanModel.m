...
%     threads_per_chain - scalar or 'auto', optional
%              Within-chain threading level. Default = 1 (off).
%              integer >= 1 sets explicit threads per chain.
%              'auto' sets threads_per_chain = floor(available_cores/chains)
%              with a minimum of 1.
%     rebuild_for_threads - bool, optional
%              If true and threads_per_chain > 1, run CmdStan clean and
%              make build STAN_THREADS=true before recompiling model.
%              Default = false.
...
classdef StanModel < handle
   ...
   methods
      ...
      function set(self,varargin)
         ...
         p.addParamValue('threads_per_chain',self.threads_per_chain,...
            @(x) (isnumeric(x) && isscalar(x) && (x >= 1)) || ...
            (ischar(x) && strcmpi(strtrim(x),'auto')) || ...
            (isa(x,'string') && isscalar(x) && strcmpi(strtrim(char(x)),'auto')));
         p.addParamValue('rebuild_for_threads',self.rebuild_for_threads,...
         ...
         threads_opt = p.Results.threads_per_chain;
         if isa(threads_opt,'string')
            threads_opt = char(threads_opt);
         end
         if ischar(threads_opt)
            self.threads_per_chain = 'auto';
         else
            self.threads_per_chain = max(1,round(threads_opt));
         end
         self.rebuild_for_threads = p.Results.rebuild_for_threads;
         
         % Keep CmdStan runtime args in sync with threading settings.
         self.configure_threading_params(false);
      end
         ...
         if ~isempty(control)
            ...
            for i = 1:numel(fn)
               switch lower(fn{i})
                  ...
                  case {'max_treedepth' 'max_depth'}
                     set_nuts_max_depth(self,control.(fn{i}));
                  otherwise
         ...
         switch lower(self.method)
            ...
            case 'sample'
               ...
               if strncmp(self.algorithm,'hmc',3)
                  ...
                  control.max_treedepth = self.params.sample.hmc.nuts.max_depth;
               end
            case 'variational'
               control = [];
         end
      ...
      function fit = sampling(self,varargin)
         ...
         self.set(varargin{:});
         self.method = 'sample';
         threads = self.configure_threading_params(true);
         self.ensure_model_compiled(threads);
         thread_env_cleanup = self.configure_threading_env(threads); %#ok<NASGU>
      ...
      function ver = stan_version(self)
         ...
         if isprop(p,'stdout') && ~isempty(p.stdout)
            txt = strjoin(p.stdout,' ');
         end
         if isprop(p,'stderr') && ~isempty(p.stderr)
            txt = [txt ' ' strjoin(p.stderr,' ')];
         end
         
         % CmdStan/stanc output format varies by version/platform.
         tok = regexp(txt,'(\d+)\.(\d+)\.(\d+)','tokens','once');
         if isempty(tok)
            tok2 = regexp(txt,'(\d+)\.(\d+)','tokens','once');
            if ~isempty(tok2)
               tok = {tok2{1} tok2{2} '0'};
            end
         end
         ...
         if ~isempty(tok)
            ver = [str2double(tok{1}) str2double(tok{2}) str2double(tok{3})];
            return;
         end
         
         % Conservative fallback: choose an API level where stansummary exists.
         if exist(fullfile(self.stan_home,'bin','stansummary'),'file')
            ver = [2 8 0];
         else
            ver = [2 7 0];
         end
      ...
      function compile(self,target,flags)
         ...
         if ischar(flags)
            flags = {flags};
         elseif isa(flags,'string')
            flags = cellstr(flags);
         elseif isempty(flags)
            flags = {};
         end
         assert(iscell(flags),'StanModel:compile:InputFormat',...
            'flags must be a char or cell array of chars');
...
%       end
   ...
   methods(Access = private)
      function threads = resolve_threads_per_chain(self,warn_user)
         if nargin < 2
            warn_user = false;
         end
         
         n_processors = java.lang.Runtime.getRuntime.availableProcessors;
         n_chains = max(1,round(self.chains));
         if ischar(self.threads_per_chain)
            if strcmpi(strtrim(self.threads_per_chain),'auto')
               threads = max(1,floor(n_processors / n_chains));
               if warn_user && (threads == 1)
                  warning('stan:threads_per_chain:auto',...
                     ['threads_per_chain=''auto'' resolved to 1 (no within-chain ' ...
                      'parallelization) based on chains and available cores.']);
               end
            else
               error('StanModel:threads_per_chain:InputFormat',...
                  'threads_per_chain must be >=1 or ''auto''');
            end
         else
            threads = max(1,round(self.threads_per_chain));
         end
         
         if warn_user && (threads > 1) && (n_chains * threads > n_processors)
            warning('stan:threads_per_chain:oversubscribe',...
               ['chains * threads_per_chain exceeds available cores. ',...
                'This may oversubscribe the CPU and reduce performance.']);
         end
      end
      
      function threads = configure_threading_params(self,warn_user)
         if nargin < 2
            warn_user = false;
         end
         
         threads = self.resolve_threads_per_chain(warn_user);
         if self.is_stan_version_at_least([2 28 0]) && (threads > 1)
            self.params.num_threads = threads;
         else
            if isfield(self.params,'num_threads')
               self.params = rmfield(self.params,'num_threads');
            end
         end
      end
      
      function cleanupObj = configure_threading_env(self,threads)
         cleanupObj = [];
         if threads <= 1
            return;
         end
         
         if self.is_stan_version_at_least([2 28 0])
            return;
         end
         
         old_threads = getenv('STAN_NUM_THREADS');
         setenv('STAN_NUM_THREADS',num2str(threads));
         cleanupObj = onCleanup(@() setenv('STAN_NUM_THREADS',old_threads));
      end
      
      function ensure_model_compiled(self,threads)
         if threads > 1
            if self.rebuild_for_threads
               self.clean_for_threading();
               self.build_for_threading();
            end
            if self.verbose
               fprintf('Compiling model with STAN_THREADS=true ...\n');
            end
            self.compile('model',{'STAN_THREADS=true'});
         else
            if ~self.is_compiled
               if self.verbose
      ...
      function clean_for_threading(self)
         clean_targets = {'clean-all','clean'};
         for i = 1:numel(clean_targets)
            command = ['make ' clean_targets{i}];
            p = processManager('id',['clean-' clean_targets{i}],...
                               'command',command,...
                               'workingDir',self.stan_home,...
                               'keepStdout',true,...
                               'keepStderr',true,...
                               'printStderr',self.verbose,...
                               'printStdout',self.verbose);
            p.block(0.05);
            if p.exitValue == 0
               return;
         ...
         warning('StanModel:compile:clean_failed',...
            'Unable to run make clean-all/clean before threaded compile.');
      end
      
      function build_for_threading(self)
         command = 'make build STAN_THREADS=true';
         p = processManager('id','build-threading',...
                            'command',command,...
                            'workingDir',self.stan_home,...
                            'keepStdout',true,...
                            'keepStderr',true,...
         ...
         p.block(0.05);
         if p.exitValue ~= 0
            warning('StanModel:compile:build_failed',...
               'Threaded toolchain build failed; continuing to model compile.');
         end
      end
      
      function bool = is_stan_version_at_least(self,req)
         if ischar(req)
            req = cellfun(@str2num,regexp(req,'\.','split'));
         end
         ver = self.stan_version;
         assert((numel(ver) == 3) && (numel(req) == 3),...
            'StanModel:version:InputFormat',...
      ...
      function set_nuts_max_depth(self,val)
