function psyrat_checktraceplots(REL,varargin)
%View trace plots of Stan parameters and ask the user whether Stan should
% rerun or save the outputs
%
%
%To identify whether the estimates are stable, they should look like "fat,
% hairy caterpillars".
%
%Input:
% REL - output from stan after running data through psyrat_computevarcomp
%
%Optional Input:
...
if nargin == 0 || isempty(REL)
    error('REL:notfound',...
        ...
        'REL from the psyrat_computevarcomp function is needed\n',...
        'See help for psyrat_checktraceplots more information'));
end
...
if isfield(REL,'nsampling') && ~isempty(REL.nsampling)
    nsamples = REL.nsampling;
else
    nsamples = max(floor(REL.niter/2),1);
end
...
if strcmp(REL.analysis,'ic')
    ...
    tpdata = zeros(nsamples,REL.nchains);
    ...
    for nplot=1:(yplots * 3)
        ...
        if trackwhichplot == 1
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.mu(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
            ...
            ylabel(str,'FontSize',fsize,'Interpreter','none');
        ...
        elseif trackwhichplot == 2
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_u(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
        ...
        elseif trackwhichplot == 3
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_e(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
...
elseif strcmp(REL.analysis,'trt')
    ...
    for nplot=1:(yplots * xplots)
        ...
        elseif trackwhichplot == 2
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_id(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
        ...
        elseif trackwhichplot == 3
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_occ(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
        ...
        elseif trackwhichplot == 4
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_trl(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
        ...
        elseif trackwhichplot == 5
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_trlxid(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
        ...
        elseif trackwhichplot == 6
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_occxid(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
        ...
        elseif trackwhichplot == 7
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_trlxocc(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
        ...
        elseif trackwhichplot == 8
            ...
            for jj = 1:REL.nchains
                tpdata(:,jj) = REL.out.sig_err(((jj-1)*nsamples)+1:...
                    jj*nsamples,nrow);
            end
...
%tag the gui so it can be closed in  psyrat_computevarcompwarp
%Namespaced 'psyrat_' so psyrat_start's prefix-scoped teardown reaches it
%(B48). The earlier reasoning that this window can never outlive estimation
%was wrong: if the rerun prompt is dismissed with the window X, guidata on
%the now-absent handle throws in psyrat_computevarcompwarp before that
%function's own close/delete of this figure is reached, so it can survive.
tplots.Tag = 'psyrat_tplots';
