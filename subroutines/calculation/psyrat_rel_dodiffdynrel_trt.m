function out = psyrat_rel_dodiffdynrel_trt(varargin)
%Dynamic/conditional reliability SURFACE of the four-cell NONCONCURRENT
%difference-of-differences for the TWO-FACET (trials x occasions) GAUSSIAN
%design (analysis 29, family = 'gaussian'). The Gaussian sibling of
%PSYRAT_REL_DODIFFDYNREL_TRT_GAMMA_LS. Derivation:
%SCIENTIFIC_FORMULA_AUDIT.md section 26.
%
% out = psyrat_rel_dodiffdynrel_trt('b',b,...,'sd_occ',sd_occ,...
%   'nocc',nocc,'reltype',reltype,...)
%
%The two-facet twin of PSYRAT_REL_DODIFFDYNREL - read that file first for the
%TYPICAL-PERSON estimand statement and the identity-link simplifications.
%This twin adds the seven-component structure and the three decompositions of
%the per-participant twin PSYRAT_SSREL_DODIFFDYNREL_TRT: reltype 1 = CE,
%2 = CS, 3 = CES, with the reltype-invariant draw-level invalidity flag.
%
...
%
%See also PSYRAT_REL_DODIFFDYNREL, PSYRAT_SSREL_DODIFFDYNREL_TRT,
%PSYRAT_DOD_COMPOSITE, PSYRAT_REL_DODIFFDYNREL_TRT_GAMMA_LS.
...
for coupled = {'rho','rho_e','cov_e_obs','cov_res_e_obs','er_cov','rescor','copula'}
    if any(strcmpi(coupled{1},varargin(1:2:end)))
        error('psyrat_rel_dodiffdynrel_trt:noresidualcoupling',... %Error code and associated error
...
for a = 1:nz1
    for c = 1:nz2
        ...
        %the same pre-merge-review rationale as the gamma sibling.
        dall = struct();
        ...
        %construction, matching the per-participant twin)
        tol = 1e-10;
