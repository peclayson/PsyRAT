function out = psyrat_rel_dodiffdynrel_gamma_ls(varargin)
%Dynamic/conditional reliability SURFACE of the four-cell NONCONCURRENT
%difference-of-differences for the gamma location-scale family, as a function
%of the standardized dimension predictor(s) (analysis 28, family = 'gamma',
%gammascale = 2).
%
% out = psyrat_rel_dodiffdynrel_gamma_ls('b',b,'b_sigma',b_sigma,...
%   'b_dim',b_dim,'b_sigma_dim',b_sigma_dim,'sd_id',sd_id,'cor_id',cor_id,...
%   'sd_trl',sd_trl,'cor_trl',cor_trl,'ndim',ndim,'z1',z1axis,...
%   'obs',nprime,'CI',.95)
%
%THE ESTIMAND, STATED BECAUSE IT DIFFERS FROM BOTH NEIGHBOURS. This surface
%is the TYPICAL-PERSON read-out: every person effect - location AND scale -
%is at zero, so the residual at z is the population sigma_q(z) =
%exp(b_sigma_q + b_sigma_dim_q . X(z)) exactly. Owner-approved 2026-08-12.
%It is NOT analysis 12/13's surface estimand (which MARGINALIZES the residual
%over person scale heterogeneity via s_sd), and it has NO counterpart in the
%owner's reference bundle (which computes only per-participant tables; see
%PSYRAT_SSREL_DODIFFDYNREL_GAMMA_LS, the reference-anchored primary output).
%Label any display of this surface as the typical-person reference curve.
%
%Everything else - the four-cell component matrices, the signed composite,
%the harmonic count rule, the coefficient formulas, and the nonconcurrent
%zero-residual-covariance boundary - is identical to the per-participant
%calculator, evaluated at grid z instead of participant z.
%
%Inputs (name-value; cells in dodmap constituent order)
% b, b_sigma      - [ndraws x 4] ABSOLUTE per-cell intercepts (offsets added)
% b_dim, b_sigma_dim - [ndraws x 4 x KDIM]
% sd_id           - [ndraws x 8]; cor_id - [ndraws x 8 x 8]
...
%Output struct out
% z1 (, z2)       - grid axes
% G, D            - DoD surfaces (ll/pt/ul); ndim = 1 -> 1 x nz1 vectors,
%                   ndim = 2 -> nz1 x nz2 matrices
% ICCg, ICCd      - matched-facet ICC surfaces at n' = 1
% sige_pt         - posterior-mean residual CONTRAST SD at z,
%                   sqrt(max(w'*Sigma_pi_e(z)*w, 0))
% invalid_frac    - per-grid-point fraction of flagged draws
% estimand        - 'typical person (all person effects at zero)'
%
%See also PSYRAT_SSREL_DODIFFDYNREL_GAMMA_LS, PSYRAT_DOD_COMPOSITE,
%PSYRAT_GAMMA_DOD_COMPONENTS_LS, PSYRAT_REL_DIFFDYNREL_GAMMA_LS.
...
for coupled = {'rho','rho_e','cov_e_obs','cov_res_e_obs','er_cov','rescor','copula'}
    if any(strcmpi(coupled{1},varargin(1:2:end)))
        error('psyrat_rel_dodiffdynrel_gamma_ls:noresidualcoupling',... %Error code and associated error
