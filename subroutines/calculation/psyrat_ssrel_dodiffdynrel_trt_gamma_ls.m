function [ssrel,drawlevel] = psyrat_ssrel_dodiffdynrel_trt_gamma_ls(varargin)
%Per-participant person-specific dynamic NONCONCURRENT difference-of-
%differences reliability for the TWO-FACET (trials x occasions) gamma
%location-scale design (analysis 29, ic_dodiff_dynrel_sserrvar_trt,
%family = 'gamma', gammascale = 2).
%
%[ssrel,drawlevel] = psyrat_ssrel_dodiffdynrel_trt_gamma_ls('b',b,...)
%
%WHAT THIS IS. The two-facet twin of PSYRAT_SSREL_DODIFFDYNREL_GAMMA_LS. Read
%that file first: the four-cell signed contrast, the nonconcurrent boundary
%(all six observation-level residual covariances fixed to zero), the
%population-signal / person-residual split, and the two D-study designs per
%participant are identical. What the second facet adds:
%
% (1) SEVEN COMPONENT MATRICES (p, i, o, pi, po, io, pio_e) instead of three,
%     with the residual pooled at the HIGHEST order only: pio_e = the
%     three-way mean-surface remainder + sigma_qp^2. Person x trial and
%     person x occasion stay separate components with their own divisors,
%     exactly as PSYRAT_DIFFREL_TRT treats the two-event design (and as the
%     reference derives; the split is the paper's own two-facet
%     decomposition).
%
% (2) THREE DECOMPOSITIONS, selected by reltype and mapped onto the
%     reference's names (frozen in the fixtures): reltype 1 = CE (the scaled
...
%     Because CE/CS universes carry a SCALED term, the universe itself
%     differs between the actual and standardized designs, so the table
%     carries uni_pt AND uni_pt_std.
%
% (3) THE INVALIDITY FLAG IS A PROPERTY OF THE DRAW, NOT OF THE SELECTED
%     DECOMPOSITION. Mirroring the reference, a draw is flagged when ANY of
%     the six scaled terms is materially negative or ANY of the three
%     decompositions' errors/coefficients is out of range - so the flag is
%     reltype-invariant and comparable across output surfaces.
%
%INPUTS - as the one-facet twin, plus:
...
%
%OUTPUTS - as the one-facet twin; the coefficient columns are the selected
%reltype's. drawlevel additionally carries G/D for ALL THREE decompositions
%(G_ces/G_ce/G_cs and D_*) for external verification against the reference.
%ssrel carries TWO floor-disclosure columns rather than the twin's one --
%n_floored for the trial counts and n_floored_occ for the occasion counts --
%because this design floors both facets and they are not interchangeable.
%
%See also PSYRAT_SSREL_DODIFFDYNREL_GAMMA_LS, PSYRAT_DOD_COMPOSITE,
%PSYRAT_GAMMA_DOD_COMPONENTS_LS, PSYRAT_DIFFREL_TRT.
...
for coupled = {'rho','rho_e','cov_e_obs','cov_res_e_obs','er_cov','rescor','copula'}
    if any(strcmpi(coupled{1},varargin(1:2:end)))
        error('psyrat_ssrel_dodiffdynrel_trt_gamma_ls:noresidualcoupling',... %Error code and associated error
...
for c = 1:nsub
    ...
    %cell would trip psyrat_dod_composite's n > 0 guard and kill the whole
    %per-participant summary. ONLY zero is floored -- deliberately NOT
    %max(1,...): MATLAB's two-input max omits NaN, so a max floor would
    %silently convert corrupt counts (NaN, negative) to n = 1 and hide
    %them from the composite guard, which must keep erroring loudly on
    %them (see the one-facet twin and psyrat_dynrel_summary's
    %reject-before-floor rationale). The floor applies to the composite
    %inputs only; ssrel keeps the true counts, and preflight
    ...
    %B16 residual, as in the one-facet twin: disclose the substitution on the
    %exported row. Trials and occasions are counted SEPARATELY -- they are
    %different facets and a reader needs to know which one was empty. Counted
    %before each floor; the trls*/occs* columns keep their true zeros.
    ...
    %the person expected score marginalizes over every non-person facet
    v_other = sd_trl.^2 + sd_occ.^2 + sd_tid.^2 + sd_oid.^2 + sd_to.^2;
    sig_own = exp(lsig_own);
    E_own = exp(mu_own + 0.5 .* v_other);
...
%then the three decompositions of the reference (reliability.R:301-323).
%The invalid flag pools every term and every decomposition - a property of
%the draw, not of the selected reltype.
...
%Matched-facet ICCs at n' = 1: unscaled composites (reliability.R:344-355).
