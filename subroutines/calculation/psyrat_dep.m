function [ll,pt,ul] = psyrat_dep(varargin)
%Calculate dependability using standard deviation draws from CmdStan
%
%[ll,pt,ul] = psyrat_dep('bp',sig_u,'wp',sig_e,'obs',[1 50],'CI',.95)
%
%
%Inputs
...
if ~isempty(varargin)
    ...
    if ~isempty(ind)
        ...
        'Please input bp. See help psyrat_dep for more information \n'));
    end
    ...
    if ~isempty(ind)
        ...
        'Please input wp. See help psyrat_dep for more information \n'));
    end
    ...
    if ~isempty(ind)
        ...
        'Please input obs. See help psyrat_dep for more information \n'));
    end
    ...
    if ~isempty(ind)
        ...
        if ciperc > 1 || ciperc < 0
            error('varargin:ci',... %Error code and associated error
                ...
                ' is invalid\n',...
                'See help psyrat_dep for more information \n'));
        end
    else 
        error('varargin:ci',... %Error code and associated error
            strcat('WARNING: Size of credible interval not specified \n\n',... 
            'Please input CI. See help psyrat_dep for more information \n'));
    end
...
elseif length(obs) == 2
    ...
    %if obs contains two numbers calculate the dependability for the range
    %specified
    ...
    for i = obs(1):obs(2)
        ...
        %align to obs(1):obs(2) (avoids leading zeros when obs(1) > 1; this
        %is identical to the previous behavior when obs(1) == 1)
        idx = i - obs(1) + 1;

        %pull the lower limit, point estimate, and upper limit
        ll(idx) = quantile(dep,ciedge);
        pt(idx) = mean(dep);
        ul(idx) = quantile(dep,1-ciedge);
