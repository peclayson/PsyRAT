function inctrltable = psyrat_trt_relcutofft(varargin)
...
for gloc=1:ngroups
    for eloc=1:nevents
        ...
        rel{end+1} = sprintf(' %0.2f CI [%0.2f %0.2f]',...
            psyrat_data.relsummary.group(gloc).event(eloc).relcutoff.m,...
            psyrat_data.relsummary.group(gloc).event(eloc).relcutoff.ll,...
            psyrat_data.relsummary.group(gloc).event(eloc).relcutoff.ul);
...
if gui == 1
    ...
    psyrat_inctrl= psyrat_newfigure('unit','pix',...
      ...
      'Tag','psyrat_output',...
      'name',...
      ['Results of Increasing the ' L.NumberOf ' on '...
      rel_name],...
      'numbertitle','off',...
      'resize','off');
...
elseif strcmp(ext,'.csv')
    ...
    %G47 sweep (latent hardening): this loop was the last remaining
    %mixed-conversion write over cell-backed table columns -- the same trap
    %class as the overall-table writers, where a char cell pushed through
    %%d prints each character's numeric code and recycles the format. No
    %reachable route feeds this table a char cell in column 2 today; the
    %sibling psyrat_depcutofft was already hardened for the same class.
    %Numeric cells keep the exact former conversion, and the row writes
    %through ONE %s so fprintf's argument-flattening cannot lose fields.
    for i = 1:height(incltrltable)
        v2 = incltrltable{i,2};
        if iscell(v2); v2 = v2{1}; end
        if ischar(v2)
            c2 = v2;
        else
            c2 = sprintf('%d', v2);
        end
        fprintf(fid,'%s\n',strjoin({char(incltrltable{i,1}), c2, ...
            char(incltrltable{i,3})}, ','));
    end
