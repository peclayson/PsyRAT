function dataout = psyrat_readtable(varargin)
%Load data into table by cycling through possible delimiters
%
%psyrat_readtable('file','J:\Data\MyData.xlsx')
%
%
%
%Input
...
if ~isempty(varargin)
    ...
    if mod(length(varargin),2)  
        ...
        'See help psyrat_readtable for more information on inputs'));
    end
    ...
    %check if a delimiter was specified (the key was misspelled 'idcol' until
    %the 2026-09-05 audit, so the documented option was unreachable)
    ind = find(strcmpi('delimiter',varargin),1);
    if ~isempty(ind)
...
if sum(strcmp(ext,supportedfiles)) ~= 1
    ...
    'For a list of supported file types, see help psyrat_readtable \n'));
